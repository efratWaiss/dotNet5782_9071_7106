...

        {
            ...
            return (from p in personsRootElem.Elements()
                    select new Parcel()
                    {
                        ...
                        Weight = (WeightCategories)Enum.Parse(typeof(WeightCategories), p.Element("Weight").Value),
                        priority = (Priorities)Enum.Parse(typeof(Priorities), p.Element("Priorities").Value),
                        Requested = string.IsNullOrEmpty(p.Element("Requested").Value) ? (DateTime?)null : DateTime.Parse(p.Element("Requested").Value),
                        DroneId = Int32.Parse(p.Element("DroneId").Value),
                        scheduled = string.IsNullOrEmpty(p.Element("scheduled").Value) ? (DateTime?)null : DateTime.Parse(p.Element("scheduled").Value),
                        PickedUp = string.IsNullOrEmpty(p.Element("PickedUp").Value) ? (DateTime?)null : DateTime.Parse(p.Element("PickedUp").Value),
                        Delivered = string.IsNullOrEmpty(p.Element("Delivered").Value) ? (DateTime?)null : DateTime.Parse(p.Element("Delivered").Value)
                        //    Delivered = Convert.ToDateTime(p.Element("Delivered").Value)
                        //                DateTime ? date = string.IsNullOrEmpty(elem.Value) ? (DateTime?)null : DateTime.Parse(elem.Value);
                        //}
                    }
        ...
        public IEnumerable<Parcel> ParcelNoDrone()
        {
            ...
            return (from p in PNoD
                    select new Parcel()
                    {
                        ...
                        Requested = Convert.ToDateTime(p.Element("Requested").Value.ToString()),
                        DroneId = Int32.Parse(p.Element("DroneId").Value),
                        scheduled = Convert.ToDateTime(p.Element("Requested").Value.ToString()),
                        PickedUp = Convert.ToDateTime(p.Element("Requested").Value),
                        Delivered = Convert.ToDateTime(p.Element("Requested").Value)
                    }
                   );
        }
        [MethodImpl(MethodImplOptions.Synchronized)]
